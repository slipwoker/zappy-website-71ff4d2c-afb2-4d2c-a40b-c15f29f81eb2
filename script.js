document.addEventListener('DOMContentLoaded', function () {
  ...
  document.body.addEventListener('submit', function (e) {
    ...
    form.querySelectorAll('[data-invalid]').forEach(function (el) {
    ...
    fields.forEach(function (field) {
      ...
      const fieldLabel = label ? label.textContent.trim().replace('*', '').trim() : name;
      ...
      if (field.hasAttribute('minlength')) {
        ...
        if (value.length < min) {
          showError(field, fieldLabel + ' must be at least ' + min + ' characters.');
        }
      }
      ...
      if (field.hasAttribute('maxlength')) {
        ...
        if (value.length > max) {
          showError(field, fieldLabel + ' must not exceed ' + max + ' characters.');
        }
      }

      if (field.tagName === 'SELECT' && (value === '' || value === 'default')) {
        if (field.hasAttribute('required')) {
          showError(field, 'Please select an option for ' + fieldLabel + '.');
        }
      }
    });
    ...
    if (isValid) {
      ...
      successMsg.style.cssText = 'padding:12px 16px;background:#c6f6d5;color:#276749;border-radius:6px;margin-top:12px;font-size:0.95em;';
    ...
    mutations.forEach(function (mutation) {
      mutation.addedNodes.forEach(function (node) {
        ...
        if (node.querySelectorAll) {
          const nested = node.querySelectorAll('.' + OBSERVE_CLASS);
          if (nested.length) initScrollAnimations();
        }
      });
    });
  });
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "הולנדיה משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
    ...
    (function() {
        ...
        function initContactForm() {
            ...
            try {
                ...
                const payload = {
                    websiteId: '71ff4d2c-afb2-4d2c-a40b-c15f29f81eb2',
                    name: name,
                    email: email,
                    subject: subject,
                    message: message,
                ...
                if (result.success) {
                    ...
                    if (result.thankYouPagePath && result.ticketNumber) {
                        ...
                        if (isPreviewMode && window.ZAPPY_CONFIG) {
                            ...
                            const websiteId = window.ZAPPY_CONFIG.websiteId || '71ff4d2c-afb2-4d2c-a40b-c15f29f81eb2';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '71ff4d2c-afb2-4d2c-a40b-c15f29f81eb2';
